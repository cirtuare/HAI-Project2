...
final class GraphViewModel {
    ...
    /// True when personas could not reach consensus and the human must answer clarifying questions.
    var debateNeedsHumanInput: Bool = false
    /// Clarifying questions generated by the personas for the human to answer.
    var debateReverseQuestions: [String] = []
    /// The human's typed answer to the reverse questions.
    var debateHumanAnswer: String = ""

    // ── Feedback slider state ─────────────────────
    /// User-expressed preference between persona A (0.0) and persona B (1.0).
    var debateFeedbackSlider: Double = 0.5
    ...
    /// Returns a formatted summary of the top nodes belonging to a persona (for debate context).
    @MainActor
    private func nodesSummary(for personaID: String, limit: Int = 20) -> String {
        let personaNodes = nodes
            .filter { $0.personaIDs.contains(personaID) }
            .sorted { ($0.isImportant ? 1 : 0) > ($1.isImportant ? 1 : 0) }
            .prefix(limit)
        guard !personaNodes.isEmpty else { return "(등록된 노드 없음)" }
        return personaNodes.enumerated().map { i, node in
    ...
    func triggerManualDebate(personaTypes: [PersonaType], query: String) {
        // Build per-persona node summaries before leaving MainActor
        var personaSummaries: [PersonaType: String] = [:]
        for type in personaTypes {
            if let record = personas.first(where: { $0.personaType == type }) {
                personaSummaries[type] = nodesSummary(for: record.id)
            }
        }

        liveDebateTurns = []
        activeDebateResult = nil
        debateStatus = "토론 준비 중..."       // prevents immediate "failed" flash in LiveDebateView
        isLiveDebatePresented = true
        debateNeedsHumanInput = false
        debateReverseQuestions = []
        debateHumanAnswer = ""
        debateFeedbackSlider = 0.5
        let ctx = modelContext
        Task {
    ...
    /// Submit the human's answer to the reverse questions and resume debate synthesis.
    @MainActor
    func submitDebateHumanAnswer() {
        let answer = debateHumanAnswer
        guard !answer.trimmingCharacters(in: .whitespaces).isEmpty else { return }
        let ctx = modelContext
        Task {
            await DebateOrchestrator.shared.continueManualDebateWithAnswer(
                answer: answer,
                context: ctx,
                viewModel: self
            )
    ...
    /// Reset all debate state (HITL mode, feedback slider, results).
    @MainActor
    func resetDebate() {
        debateNeedsHumanInput = false
        debateReverseQuestions = []
        ...
        debateFeedbackSlider = 0.5
        activeDebateResult = nil
        activeDebateTranscript = []
        liveDebateTurns = []
        debateStatus = nil

        let records = (try? modelContext.fetch(FetchDescriptor<DebateRecord>())) ?? []
        for r in records where r.status == "awaiting_human" {
            modelContext.delete(r)
        }
        try? modelContext.save()

        Task { await DebateOrchestrator.shared.clearPendingHITL() }
    }
